...
typedef struct BST_Node_Struct
{
     ...
     ***/
    struct BST_Node_Struct *left; // Left child
    struct BST_Node_Struct *right; // Righg child
} BST_Node;
...
BST_Node *newBST_Node(double freq, int bar, double index)
{
    ...
    if(index >= 0.0 && index < 1.0)
        ...
        printf("Index should be between 1 and 0");
...
BST_Node *BST_insert(BST_Node *root, BST_Node *new_node)
{
     ...
     * 
     * And it must return without inserting anything in the
     * BST.
     */
...
BST_Node *BST_search(BST_Node *root, int bar, double index)
{
     ...
     ****/
    double search_key = (10.0 * bar) + index;
...
BST_Node *find_successor(BST_Node *right_child_node)
{
     ...
     * Implement this function
     ****/
    BST_Node *successor = right_child_node;
...
BST_Node *BST_delete(BST_Node *root, int bar, double index)
{
    ...
    double deleting_key = (10.0 * bar) + index;
    if (root == NULL) // Tree or subtree empty
    {
        return NULL;
    }
    if(root->key == deleting_key)
    {
        if(root->left == NULL && root->right == NULL) // case a) Node with no children
        {
            free(root);
            return NULL;
        }
        else if(root->left == NULL) // case b) Node with 1 child, right subtree.The parent will \
                                    be deleted and right subtree becomes the new_parent
        {
            new_parent = root->right;
            free(root);
            return new_parent;
        }
        else if(root->right == NULL) // case b) Node with 1 child, left subtree.The parent will \
                                    be deleted and left subtree becomes the new_parent
        {
            new_parent = root->left;
            free(root);
            return new_parent;
        }
        else // case c) Node with 2 children
        {
...
void BST_shiftFreq(BST_Node *root, char note_src[5], char note_dst[5])
{
    ...
    int src_position = 0;
    int dst_position = 0;
    while(strcmp(note_names[src_position], note_src) != 0)
    {
        src_position++;
    }
    double frequency1 = note_freq[src_position];
    while(strcmp(note_names[dst_position], note_dst) != 0)
    {
        dst_position++;
    }
    double frequency2 = note_freq[dst_position];
    if(frequency1 == root->freq)
    {
        root->freq = frequency2;
    }
    else
    {
        if(root->left != NULL) BST_shiftFreq(root->left, note_src, note_dst);
        if(root->right != NULL) BST_shiftFreq(root->right, note_src, note_dst);
    }
}   
...
BST_Node *Harmonizing(BST_Node *head, BST_Node *root, int semitones, double time_shift) // Helper function
{
    int position = 0;
    BST_Node *harmonize_helper = NULL; 
    if (root == NULL) 
    {
        return NULL;
    }
    if(root->left != NULL)
    {
        harmonize_helper = Harmonizing(head, root->left, semitones, time_shift);
    }
    if (root->right != NULL) 
    {
        harmonize_helper = Harmonizing(head, root->right, semitones, time_shift);
    }
    while(position < 100)
    {
        if (note_freq[position] == root->freq && (position + semitones) >= 0 && (position + semitones) <= 99)
        {   
            if ((root->index + time_shift) >= 0.0 && (root->index + time_shift) < 1.0)
            {
                BST_Node *new_note = newBST_Node(note_freq[position + semitones], root->bar, (root->index + time_shift));
                BST_Node *searching = BST_search(root, new_note->bar, new_note->index);
                if (searching == NULL) 
                {
                    harmonize_helper = BST_insert(root, new_note);
                }
                else
                {
                    while(searching != NULL)
                    {   
                        harmonize_helper->index = harmonize_helper->index + (1*10^-6);
                        harmonize_helper->key = harmonize_helper->key + (1*10^-6);
                        searching = BST_search(root, new_note->bar, new_note->index);
                    }
                    if (harmonize_helper->index < 0.0 && harmonize_helper->index >= 1.0) 
                    {
                        free(new_note); // deleting the note with index out of range
                    }
                    else
                    {
                        harmonize_helper = BST_insert(root, new_note);
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
     ...
     *    immediately next one in the musical scale - it what is the
     ...
     * Implement this function
     ****/
    root = Harmonizing(root, root, semitones, time_shift); // Getting the updated tree
    return root;
}
